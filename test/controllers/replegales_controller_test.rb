...
class ReplegalesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create replegal" do
    ...
    assert_difference('Replegal.count') do
      post replegales_url, params: { replegal: {nombre_completo: "Leo Moreno", rfc: "MOUL8902072X2", cliente_ids: ["",clientes(:one).id] }, clienteasociado: {cliente_id: clientes(:two).id} }
    end
  ...
  test "should update replegal" do
    sign_in_as(@usuario,"password")
    patch replegal_url(@replegal), params: { replegal: {nombre_completo: "Leo Moreno", rfc: "MOUL8902072X2", cliente_ids: ["",clientes(:one).id]}, clienteasociado: {cliente_id: clientes(:two).id}  }
