...
class ReplegalesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create replegal" do
    ...
    assert_difference('Replegal.count') do
      post replegales_url, params: { replegal: {nombre_completo: "Leo Moreno", rfc: "MOUL8902072X2" }, clientes: {cliente_id: ["",clientes(:one).id]} }
    end
  ...
  test "should update replegal" do
    sign_in_as(@usuario,"password")
    patch replegal_url(@replegal), params: { replegal: {nombre_completo: "Leo Moreno", rfc: "MOUL8902072X2" }, clientes: {cliente_id: ["",clientes(:one).id]}  }
